...
- **MAPE explodes at long horizons but MedAPE stays stable.** The aggregate MAPE for all models exceeds 0.45, driven by long-horizon outliers (SNOW two-year filing gaps). The by-bucket slice in §4b separates this.
...
LightGBM MedAPE 0.314 vs Naive 0.180 on the 9-ticker (8 scored) universe is the most important single finding to read carefully. It is not a problem with the model or the wiring; it is the honest difficulty of forecasting persistent annual financial metrics on a small cross-section. Two reasons converge.

Annual EPS is highly persistent year over year for the kind of mid-to-large-cap, stable-fundamentals tickers in the fixture, so last-year-actual is a genuinely strong baseline. Beating it by a real margin requires either a cross-section large enough for a tree model to learn signal that generalises past naive persistence, or per-cell noise floor measurement that tells us whether a small win is real signal or sample noise. Eight scored tickers is far too few for the first; the noise-floor program (S15) that addresses the second is not built in v1.0. The resolution path is `IDEA-S18-001` (cross-section expansion) and `S15` (per-cell sigma), in that order.
